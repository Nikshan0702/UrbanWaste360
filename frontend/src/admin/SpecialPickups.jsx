...
// // expects authFetch, isAdmin, isCollector, refreshPickups, fetchOutstanding, fetchHistory, collectors via props or context
...
//     try {
//       const r = await authFetch(`/api/pickups/${id}/status`, {
//         method: 'PUT',
//         userId: user.id,
//         body: { status: 'CANCELLED', notes: reason },
//       });
//       if (r.ok) { await refreshPickups(); alert('Rejected'); }
//       else alert(await r.text());
//     } catch { alert('Reject failed'); }
//   };

//   const approveAndAssign = async (id, currentPrice) => {
//     if (!selectedCollectorId) return alert('Select a collector');
//     const priceInput = prompt('Enter price (LKR) for this pickup:', String(Number(currentPrice || 0).toFixed(2)));
...
//     try {
//       setApproving(true);
//       const r = await authFetch(`/api/pickups/${id}/approve-assign`, {
//         method: 'PUT',
//         userId: user.id,
//         body: { crewId: selectedCollectorId, price },
//       });
//       if (r.ok) {
//         await Promise.all([refreshPickups(), fetchOutstanding()]);
//         alert('Approved & assigned. Charge added to outstanding.');
//       } else {
//         alert(await r.text());
//       }
//     } catch {
//       alert('Approve & assign failed');
//     } finally {
//       setApproving(false);
//     }
//   };

//   const assignPickup = async (id) => {
//     if (!selectedCollectorId) return alert('Select a collector');
//     try {
//       setAssigning(true);
//       const r = await authFetch(`/api/pickups/${id}/assign?crewId=${encodeURIComponent(selectedCollectorId)}`, {
//         method: 'PUT',
//         userId: user.id,
//       });
//       if (r.ok) { await refreshPickups(); alert('Assigned'); }
//       else alert(await r.text());
...
//       });
//       if (r.ok) {
//         await Promise.all([refreshPickups(), fetchHistory()]);
//         alert('Marked completed.');
//       } else {
//         alert(await r.text());
//       }
//     } catch { alert('Complete failed'); }
//   };
...
//         icon={<FaHistory className="text-emerald-600" />}
//         right={
...
//               className="px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-sm"
//             >
//               Refresh
//             </button>
//           </div>
...
//       <CardBody>
//         {pickups.length === 0 ? (
//           <div className="text-center text-gray-500 py-6">No special pickups</div>
//         ) : (
...
//                       {p.wasteType} • {p.status}
//                     </div>
//                     <div className="text-sm text-gray-600">
//                       {p.pickupDate} {p.pickupTime ? `at ${p.pickupTime}` : ''}
//                     </div>
//                     <div className="text-sm text-gray-600">
//                       Price: LKR {Number(p.price || 0).toFixed(2)}
//                     </div>
//                     {p.status === 'APPROVED' && (
//                       <div className="text-sm text-emerald-700">
//                         Approved • LKR {Number(p.price || 0).toFixed(2)} added to Outstanding
//                       </div>
//                     )}
//                   </div>
...
//                       <>
//                         <button
//                           disabled={!selectedCollectorId || approving}
//                           onClick={() => approveAndAssign(p.id, p.price)}
//                           className="px-3 py-1.5 rounded bg-green-600 disabled:bg-gray-400 text-white text-sm"
//                         >
...
//                     {/* Optional: manual assign without approval */}
//                     {isAdmin() && (p.status === 'SCHEDULED' || p.status === 'IN_PROGRESS') && (
//                       <button
//                         disabled={!selectedCollectorId || assigning}
//                         onClick={() => assignPickup(p.id)}
//                         className="px-3 py-1.5 rounded bg-purple-600 disabled:bg-gray-400 text-white text-sm"
//                       >
...
//                     {/* Collector control */}
//                     {isCollector() && (p.status === 'APPROVED' || p.status === 'IN_PROGRESS') && (
//                       <button
